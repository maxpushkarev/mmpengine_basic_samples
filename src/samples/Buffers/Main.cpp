	...
	PSTR, std::int32_t)
{
	...
	const auto userApp = std::make_shared<Sample::Buffers::App>();
	const auto rootApp = MMPEngine::Frontend::App::BuildRootApp(appContextSettings, userApp);
	rootApp->GetContext()->logger = std::make_unique<MMPEngine::Frontend::Win::OutputLogger>("Buffers logger");

	MMPEngine::Frontend::Win::AppContainer::Settings appContainerSettings {};
	appContainerSettings.base = {"Buffers"};
